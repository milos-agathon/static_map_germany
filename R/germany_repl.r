...
deu <- st_read("germany_lau.shp", stringsAsFactors = FALSE) %>% 
  	   st_transform(4326) %>% 
       st_as_sf()
...
p <- ggplot() +
	...
	scale_fill_manual(
		name="in kilometers", 
		values = rev(c('#440154', '#453572', '#3c5c87', '#3d8289', '#67a47a', '#90c572', '#c1e475', '#ffff8a' # I used chroma.js to mix the colors
		)),
      	drop = F)+
    ...
    theme(
    	...
    	axis.title.x = element_text(size=9, color="grey60", hjust=0, vjust=-7),
    	...
    	panel.grid.minor = element_blank(),
  		plot.title = element_text(face="bold", size=16, color="grey20", hjust=.5),
